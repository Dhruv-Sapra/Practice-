...
static pid_const_t pid_constants = {.kp = 2, .ki = 0.0, .kd = 8, .val_changed = true};
static pid_const2_t pid_constants2 = {.kp2 = 1.5, .ki2 = 0.0, .kd2 = 3, .setpoint = 22, .offset = 0.0,.x=4,.y=-2,.llp=64,.hlp=68 ,.val_changed = true}; //random values for now.
...
static esp_err_t tuning_pid_post_handler(httpd_req_t *req)
{
    ...
    if (!cJSON_HasObjectItem(root, "kp") || !cJSON_HasObjectItem(root, "ki") || !cJSON_HasObjectItem(root, "kd") || !cJSON_HasObjectItem(root, "kp2") || !cJSON_HasObjectItem(root, "ki2") || !cJSON_HasObjectItem(root, "kd2") || !cJSON_HasObjectItem(root, "x") || !cJSON_HasObjectItem(root, "y") || !cJSON_HasObjectItem(root, "llp") || !cJSON_HasObjectItem(root, "hlp") || !cJSON_HasObjectItem(root, "setpoint") )
    {
        ESP_LOGE(TAG, "invalid json response");
        return ESP_FAIL;
    }
